...
@WebServlet("/TrackRescueServlet")
public class TrackRescueServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        out.println("<html><head><title>Track Rescue Request</title>");
        ...
        out.println("<h2>Rescue Request Status</h2>");

        try (Connection con = DBConnection.getConnection()) {

            // 🔥 CHANGE TABLE NAME HERE
            String sql = "SELECT pet_type, location, status FROM rescue_request WHERE name=?";
            ...
            while (rs.next()) {
                ...
                String petName = rs.getString("pet_type");
                ...
                out.println("<p>Status: <span style='color:" + color + "; font-weight:bold;'>"
                        + status + "</span></p>");
                out.println("</div>");
            }

            if (!found) {
                out.println("<h3>No rescue requests found</h3>");
            }
